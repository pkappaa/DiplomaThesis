"""
Monthly-rebalancing portfolio gymnasium environment for SB3 PPO training.

Observation space : Box(-inf, inf, (78,), float32)
...
NO LOOKAHEAD: state at rebalancing date R[i] uses data from month M[i]-1 only.
"""
...
class PortfolioEnv(gym.Env):
    """
    Monthly-rebalancing portfolio environment.
    ...
    def _build(self, returns_path: Path) -> None:
        ...
        # Diagnostic arrays for sanity checks
        ...
        for i, reb in enumerate(self._all_reb):
            ...
            # ── current month forward returns (reward signal) ─────────────
            if i + 1 < n:
        ...
        # ── expanding normalisation (shift so date t uses only data < t) ──
    ...
    def step(self, action: np.ndarray):
        ...
        # Track negative returns for downside-std penalty
        if net_ret < 0:
        ...
        # After termination return last valid obs; SB3 VecEnv will call reset()
