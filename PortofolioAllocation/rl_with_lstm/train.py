"""
Train a PPO agent for monthly portfolio allocation — Phase 2 (RL + LSTM features).

Run from the PortofolioAllocation/ directory:
    python -m rl_with_lstm.train
or
    python rl_with_lstm/train.py
"""
...
def run_sanity_checks() -> None:
    ...
    # 2 ── observation shape ─────────────────────────────────────────────────
    obs, _ = env.reset()
    assert obs.shape == (116,), f"Expected (116,), got {obs.shape}"
    print(f"2. Obs shape      : {obs.shape}  [OK]")

    # 3 ── softmax inside step() produces valid weights ──────────────────────
    ...
    # 6 ── LSTM features sanity: first step should have neutral prior ─────────
    env.reset()
    reb0     = env._reb_dates[0]
    lm_first = env._lstm_mean.loc[reb0].values
    ls_first = env._lstm_std.loc[reb0].values
    neutral  = 1.0 / 19
    # First rebalancing date (2022-01-03) has no LSTM days before it
    # so mean should be 1/19 and std should be 0
    assert abs(lm_first.mean() - neutral) < 1e-4, (
        f"Expected neutral LSTM mean ~{neutral:.4f}, got {lm_first.mean():.4f}"
    )
    assert ls_first.sum() == 0.0, (
        f"Expected zero LSTM std at first step, got {ls_first.sum():.6f}"
    )
    print(f"6. LSTM neutral prior (step 0): mean~{lm_first.mean():.4f}, std={ls_first.sum():.4f}  [OK]")
...
def train() -> None:
    ...
    print("=" * 60)
    print("  TRAINING  — PPO + LSTM features  (SB3)")
    ...
    model.learn(
        ...
        tb_log_name        = "ppo_lstm_portfolio",
